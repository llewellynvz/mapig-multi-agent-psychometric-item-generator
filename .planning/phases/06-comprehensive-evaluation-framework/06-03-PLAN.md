...
def run_evaluation_suite(model_provider: str = "claude") -> EvaluationMetrics:
    ...
    for scale in scales:
        ...
        try:
            ...
            result = graph.invoke({"user_request": request}, config={"configurable": {"thread_id": f"eval-{scale.domain}"}})
            generated_items = result["final_output"].items if result.get("final_output") else []
