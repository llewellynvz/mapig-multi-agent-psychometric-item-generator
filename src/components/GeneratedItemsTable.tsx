...
function ValidationScoreDisplay({
  ...
  return (
    <div className="validation-score mt-3 space-y-2">
      ...
      {/* Rationale panel */}
      {expandedRationale && (
        <InsetPanel className="rounded-xl border border-white/20 bg-slate-900/45 p-3 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Rationale
          </p>
          <p className="mt-1 text-sm text-slate-100">{rationale}</p>
        </InsetPanel>
      )}
        ...
        {fullOutput?.audit?.validation_attempts && fullOutput.audit.validation_attempts > 0 && (
          <InsetPanel className="rounded-2xl p-4">
            ...
            <p className="mt-2 text-sm text-slate-100">
              Total validation attempts: {fullOutput.audit.validation_attempts}
            </p>
            <p className="text-sm text-slate-100">
              Items regenerated: {fullOutput.audit.validation_failures || 0}
            </p>
          </InsetPanel>
        )}
      </CardContent>
